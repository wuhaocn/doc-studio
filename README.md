...
- 机器主体、访问密钥与开放文档 API 已接通，支持主体启停与删除、多密钥、逐知识库 `READ/WRITE` 作用域、密钥重新生成、暂停、删除与并发版本保护；停用主体期间不签发新密钥，当前访问密钥只面向机器主体，不作为用户个人凭证。
...
- Web 端“开放接入”页已收口为凭证治理优先：保留当前可展示密钥、复制与重新生成，单文档保存 / 回读调试按需展开。
...
- 当前只保留 `RICH_TEXT / MARKDOWN / HTML` 三种正式格式；新写入文档由服务端统一派生 `contentText / summary / renderedHtml`。
- Web 新建文档弹窗已提供富文本帖子、Markdown 操作规范、H5 点击小程序三类差异化快速示例，开放接入调试台复用 Markdown / H5 示例；H5 示例以声明式事件由前端安全渲染点击演示，不在正文中执行任意脚本。
...
当前直接目标不再是“有没有入口”，而是继续完成 `P8` 收尾、阶段冻结和主链路产品化收口：更成熟的对话框接入封装、批量同步保护、权限细化、搜索索引化、跨端会话策略以及编辑 / 版本信任体验；当前已经进入“主链路产品化 + 内容资产公开消费”阶段。

当前收口原则：

- 不新增评论、实时协作、模板 / 标签、AI 主入口、桌面同步、任意源码级应用运行时等外围能力
- 先冻结当前批次范围，再统一 canonical 文档、主流程页面表达和冒烟验收口径
- 正文消费面继续围绕统一 `renderedHtml` 收口，入口消费面继续围绕结构化卡片收口
- 开放接入继续做“更好接入”，不反向拖乱正式发布和公开消费模型

下一步执行批次：

1. 阶段冻结：统一 README、产品基线、计划文档和冒烟清单口径
2. 主流程产品化收口：继续统一阅读、搜索、公开和知识库入口面的表达方式
3. 开放接入收尾：补齐对话框直连封装、批量同步保护和更清晰的结果反馈
4. 专题化推进：把搜索索引化、权限细化和跨端会话策略拆成独立专题推进
...
直接从 IDE 运行 `com.memora.MemoraApplication`，或执行默认启动脚本 `./start-backend.sh` 时，默认本地 H2 文件库会自动引导一个最小管理员工作区，可使用 `admin / 123456` 登录；它只补最小管理员与默认工作区，不加载完整演示文档数据，也不开放占位 bearer token。
