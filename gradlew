...
if $cygwin ; then
    ...
    ROOTDIRSRAW=`find -L / -maxdepth 1 -mindepth 1 -type d 2>/dev/null | xargs`
    SEP=""
    ...
    for arg in "$@" ; do
        ...
        if [ $CHECK -ne 0 ] && [ $CHECK2 -eq 0 ] ; then                    ### Added a condition
            eval `echo args$i`=`cygpath --path --windows "$arg"`
        else
...
function splitJvmOpts() {
    # For sh compatibility, we'll use a simpler approach
    JVM_OPTS="$1"
}
