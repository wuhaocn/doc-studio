...
- 知识库进入、创建与基础管理
- 知识库内文档树浏览与维护
- 新建文档弹窗提供富文本帖子、Markdown 操作规范、H5 点击小程序三类差异化快速示例，创建后直接进入对应编辑器
- 知识库与当前节点最近审计记录
- 知识库与文档回收站 / 恢复
- 独立文档阅读页与编辑页
- 版本查看、回滚、阅读链接复制与受控分享
...
- 机器主体启停与删除、多密钥签发、逐知识库 `READ / WRITE` 作用域编辑；停用主体需恢复后再新增密钥
- 开放接入页：凭证治理优先，保留当前密钥展示与复制；保存/回读调试按需展开
- 开放接入调试台提供 Markdown 操作规范 / H5 点击小程序快速示例，便于外部 AI 或脚本快速写入可预览正文
...
- [documentId.js](./src/utils/documentId.js)
...
- 开放接入页先给“主体生命周期 + 密钥 + 权限范围”，保存 / 回读调试按需展开
...
- 新建文档弹窗提供格式示例按钮；示例只填充首版标题、摘要和正文，不引入模板管理系统
...
- 前端认证快照只写入 `sessionStorage`，不做旧本地会话迁移。
- 登录、退出登录和工作区切换会通过 `BroadcastChannel` 同步到其他标签页；`localStorage` 仅用于草稿、搜索历史和最近知识库等当前浏览器功能。
